...
#############################################################################################
#### Generate majority-vote ensemble maps and save as geotiff raster files
#############################################################################################
...
for country in countries.keys():
    ...
    ma.calculate_mode(rasters, return_freq=False).rio.to_raster(
        os.path.join(
            INTERIM_DIR, "raster_stacks", "majority_ensemble", f"{country}_models.tif"
        )
    )
...
rasters = glob(
    os.path.join(INTERIM_DIR, "raster_stacks", "majority_ensemble", "*_models.tif")
)
rasters = merge_arrays(
    [
        (rxr.open_rasterio(raster, masked=True).squeeze())
        for raster in rasters
        if "majority_ensemble_map" not in raster
    ]
)
rasters.rio.to_raster(os.path.join(PROCESSED_DIR, "majority_ensemble_map.tif"))

# calculate pixel proportions for each class in the majority ensembles by country
proportions = pd.DataFrame()
rasters = glob(os.path.join(INTERIM_DIR, "raster_stacks", "majority_ensemble", "*.tif"))
for raster in rasters:
...
proportions.to_csv(
    os.path.join(PROCESSED_DIR, "majority_pixel_proportions.csv"), index=False
)
